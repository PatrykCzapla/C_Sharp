...
namespace lab09_a
{
    class Program
    {
        public static Func<double>  RandomNumberGenerator(int seed, int min, int max)
        {
            ...
            return new Func<double>(() => rand.Next(min,max));
        }
        ...
        static void Main(string[] args)
        {
            ...
            int k = 5;
            var transformed2 = fibonacciNumbers2.MyWhere(x=>x%k==0);
            ...
            var transformed = fibonacciNumbers1.ToTransformedArray(x => x / 2);
            ...
            transformed = fibonacciNumbers1.ToTransformedArray(x=>x=rndGen());
            transformed.MyEach(x => Console.Write(x + " "));
            Console.WriteLine();
            Console.WriteLine();
            ...
            foreach(var s in text)
            {
                ...
                {
                    if (el == "") continue;
                    if (dic.ContainsKey(el)) dic[el] += 1;
                    else
                    {
                        dic.Add(el, 1);
                    }
            ...
            var a = dic.ToList();
            a.Sort((x,y)=>x.Key.CompareTo(y.Key));
            a.MyEach(x => Console.WriteLine(x.Key+": "+x.Value));
